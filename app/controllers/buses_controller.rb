  ...
  private
    ...
    def get_bus_stops_from_bus(bus, bus_stop)
      ...
      StopPosition.where('bus_id = ?', bus.id).each do |stop_position|
        ...
        if delay.nil?
          Delay.create(stop_position_id: stop_position.id, day_id: day_id, this_hour: current_time.hour, minutes_delayed: ((current_time-predicted_time)/60).floor, precision: 1)
          delay = Delay.where('stop_position_id = ? and this_hour = ? and day_id = ?', stop_position.id, current_time.hour, day_id).first
        end
